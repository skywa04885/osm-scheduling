...
Job::Job(unsigned long aId)
    : mId(aId), mActive(false), mTasks(), mStartTime(std::nullopt),
      mEndTime(std::nullopt) {}

/// Constructor for the job with tasks.
Job::Job(unsigned long aId, std::list<std::shared_ptr<Task>> aTasks)
    : mId(aId), mActive(false), mTasks(std::move(aTasks)),
      mEndTime(std::nullopt) {}

/// Move constructor for the job.
Job::Job(Job &&aJob) noexcept
    : mId(aJob.mId), mActive(aJob.mActive), mTasks(std::move(aJob.mTasks)),
      mStartTime(aJob.mStartTime), mEndTime(aJob.mEndTime) {}
